...
describe('config-validate hooks target', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'config-validate-hooks-test-'));
  });
  ...
  it('validates tasks.json when present', async () => {
    ...
    const result = await handleConfigValidate(createCtx(tempDir), { target: 'hooks' });
  ...
  it('returns no-config message when neither file exists', async () => {
    const result = await handleConfigValidate(createCtx(tempDir), { target: 'hooks' });
    expect(result.content[0]?.text).toContain('No tasks.json or hooks.json');
  });
});

...
Run a [remote server](/server/headless) and access your workspaces from any machine — desktop app, browser, or CLI. Transfer sessions between local and remote workspaces. Enable server mode with a single toggle in Settings, or deploy a standalone server on Linux/macOS.

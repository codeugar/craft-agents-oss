...
- **Messaging: plan-token revocation keyed by binding** — Plan tokens are now revoked per binding instead of per session, preventing token reuse across rebinds.
- **Messaging: `/pair` throttling per sender** — `/pair` command consumption is throttled per sender to prevent abuse/spam.
- **Messaging/WhatsApp: pending sends drained on worker exit** — Worker now times out and drains pending sends on exit instead of dropping them silently.
- **Messaging/WhatsApp: connect-dialog listeners scoped to active workspace** — Prevents stale listeners from a previous workspace from firing.
- **Messaging: `BindingStore` changeListener fires only on successful writes** — Failed writes no longer trigger listeners, preventing UI from reacting to non-events.
- **Messaging/Telegram: non-private chats dropped at adapter boundary** — Group/channel messages are rejected early so they can't accidentally drive an agent session.

...
class ModelRefreshService {
  ...
  private async _doRefresh(slug: string): Promise<void> {
    ...
    if (connection.providerType === 'pi' && connection.modelSelectionMode === 'userDefined3Tier') {
      ...
      ipcLog.info(`Model refresh [${slug}]: preserving user-defined Pi model list (${modelCount} models)`)
      if (modelCount > 10) {
        ipcLog.warn(`Model refresh [${slug}]: userDefined3Tier has suspicious model count (${modelCount})`)
      }
      return
    }

...
# 4. Stage native deps (SDK core + arch binary alias + ripgrep + interceptor)
#    that electron-builder cannot see because they are hoisted to the ROOT
#    node_modules. Shared with build-dmg-local.sh so the two build paths can
#    never drift; see stage-native-deps.sh for the full rationale.
bash "$SCRIPT_DIR/stage-native-deps.sh" "$ARCH"
